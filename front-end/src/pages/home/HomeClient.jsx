...
export const HomeClient = () => {
  ...
  const navLinks = [
    { id: 1, name: "Inicio", page: "home" },
    { id: 2, name: "Smartphones", page: "smart" },
    { id: 3, name: "Acessorios", page: "acessorios" },
    { id: 4, name: "Tablets", page: "tablets" },
    { id: 5, name: "Contacto", page: "contacto" },
  ];
  ...
  return (
    <div className='mt-4 md:mt-10 '>
      ...
      <div className='flex flex-wrap gap-4 px-3'>
        ...
        <div className='hidden lg:block w-[15%] h-[230px] bg-white shadow-xl rounded-xl'>
          ...
          <div className='mt-4 px-2'>
            {categoriesMenu.map((category) => (
              <button key={category.id} className='w-full flex items-center justify-between gap-3 mb-3'>
        ...
        <div className='hidden lg:block w-[15%]'>
           <div className='grid grid-rows-5 gap-3'>
              {benefits.map((benefit) => (
                <div key={benefit.id} className='p-3 shadow-sm border border-[#ccc] rounded-lg bg-white/50 hover:bg-white/70 transition-all duration-300 hover:shadow-md hover:scale-105 cursor-pointer flex items-center gap-3'>
        ...
        <div className='w-full p-3'>
          <div className='grid md:grid-cols-2 lg:grid-cols-4 gap-3'>
            {Array.isArray(benefits.splice(-1)) && benefits.map((item,index) => (
              <div key={index} className='flex items-center gap-2 col-span-1 h-[70px]  border-t border border-[#ccc] px-3
              rounded-lg
              '>
      ...
      <div className='grid md:grid-cols-2 gap-4 mt-8 px-3'>
        ...
        <div className='col-span-1 min-h-[200px] border border-[#ccc] rounded-xl bg-white/50 shadow-sm p-3'>
          ...
          <div className='grid md:grid-cols-2 lg:grid-cols-4 gap-3'>
            {featuredCategories.map((category) => (
              <div key={category.id} className='p-3 border border-[#ccc] bg-cinza-claro hover:bg-white transition-all duration-300 min-h-[180px] rounded-xl flex items-center gap-3 flex-col justify-center cursor-pointer hover:shadow-lg hover:scale-105'>
        ...
        <div className='col-span-1 min-h-[200px] border border-[#ccc] rounded-xl bg-white/50 shadow-sm p-3'>
          ...
          <div className='grid md:grid-cols-2 lg:grid-cols-4 gap-3'>
            {featuredProducts.map((product) => (
              <div key={product.id} className='border border-[#ccc] bg-cinza-claro hover:bg-white transition-all duration-300 h-auto rounded-xl  cursor-pointer hover:shadow-lg hover:scale-105'>
                <div className='relative w-full  p-2 bg-white/80 rounded-tl-md rounded-tr-md flex items-center justify-center'>
                  ...
                  <div className='flex items-center justify-center absolute top-2 right-1 w-[40px] h-[25px] rounded-lg '>
                    <Heart className='size-5 font-bold'/>
                  </div>
                </div>
                <div className='w-full  p-1 rounded-bl-md rounded-br-md'>
                  <h2 className='text-md font-semibold'>{product?.name}</h2>
                  <p className='text-xl font-bold mt-1'>{formatNumber(product?.price)} {product?.currency}</p>
                  <p><del>{product.markPrice} {product?.currency}</del></p>
      ...
      <div className='grid md:grid-cols-2 gap-4 my-4 px-3 '>
        <div className='col-span-1 bg-gradient-to-r from-black via-secondary-blue to-primary-blue p-3
        min-h-[90px] rounded-xl flex flex-col lg:flex-row items-center gap-2
        '>
            <div className='flex flex-col lg:flex-row items-center gap-2 w-full'>
              <div className='w-[60px] h-[60px] flex items-center justify-center'>
               <MailOpen className='size-7 text-color-gray'/>
              </div>
            ...
            <div className='w-full md:w-auto'>
              <form className='flex'>
                <input type='text' placeholder='Seu melhor email' className='pl-3 rounded-tl-xl rounded-bl-xl outline-none
                border border-[#ccc] flex-1 md:w-auto
                '/>
                <button className='w-[130px] h-[50px] bg-primary-blue rounded-tr-xl rounded-br-xl
                 shadow-xl border border-primary-blue flex items-center justify-center text-white
                '>
                  Cadastrar
                </button>
              </form>
        ...
        <div className='col-span-1 grid lg:grid-cols-2 gap-2'>
          <div className='flex p-3 col-span-1 bg-gradient-to-tr from-secondary-blue to-primary-blue rounded-xl'>
            <div className='w-[50%] flex flex-col items-start justify-center gap-4 lg:flex-row lg:block'>
              <h1 className='text-lg font-semibold text-white leading-normal'>Lacamentos 2026</h1>
              <p className='text-sm text-white leading-normal'>Confira os artigos mais desejados do ano.</p>

              <div className='cursor-pointer transition duration-300 hover:bg-gray-400 w-[150px] p-2 border border-[#ccc] rounded-md mt-2 flex items-center justify-center'>
                <span className='text-white font-semibold text-sm'>Ver lancamentos</span>
              </div>
            </div>
          ...
          <div className='flex p-3 col-span-1 bg-gradient-to-tr from-secondary-blue via-black/80 to-color-black rounded-xl'>
            <div className='w-[50%] flex flex-col items-start justify-center gap-4 lg:flex-row lg:block'>
      ...
      {/* Footer */}
      <div className='w-full min-h-[400px] bg-color-black p-3'>
